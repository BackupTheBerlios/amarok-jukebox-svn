...
def artistsHtml(c):
    ...
    for id, name in c.artists():
        s += "<li><a href='/cgi-bin/browse.py?artist=%s'>%s</a></li>" % (id , cgi.escape(name.encode('utf-8')))
        s += "</ul>"
    return s
...
def albumsByArtistHtml(c, id):
    ...
    for id, name in c.albumsByArtist(id):
        s += "<li><input type='checkbox' name='album' value='%s' /> <a href='/cgi-bin/browse.py?album=%s'>%s</a></li>" % (id, id , cgi.escape(name.encode('utf-8')))
    s += "</ul>"
...
def songsByAlbumHtml(c, id):
    ...
    for url, title in c.songsByAlbum(id):
        s += "<li><input type='checkbox' name='song' value='%s' /> <a href='/cgi-bin/browse.py?song=%s'>%s</a></li>" % (cgi.escape(url.encode('utf-8')), urllib.quote(url.encode('utf-8')), cgi.escape(title.encode('utf-8')))
    s += "</ol>"
